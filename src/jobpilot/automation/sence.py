...
class SenceAutomator(BaseAutomator):
    ...
    def _check_logged_in(self, page) -> bool:
        try:
            ...
            if (
                page.locator("text='Cerrar Sesión'").count() > 0
                or page.locator("text='Mi Curriculum'").count() > 0
            ):
                return True
    ...
    def apply_to_job(
        ...
        try:
            ...
            confirm_button = self._page.locator(
                "button:has-text('Confirmar postulación'), button:has-text('Enviar')"
            ).first
