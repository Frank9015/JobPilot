...
class CrossPortalDeduplicator:
    ...
    def __init__(self, session: Session, threshold: float = 0.90) -> None:
        ...
        logger.debug(
            f"Deduplicator cache cargado con {len(self._recent_offers)} ofertas recientes."
        )
    ...
    def is_duplicate(
        self, portal: str, external_id: str | None, title: str, company: str | None
    ) -> bool:
        """
        ...
        for offer in self._recent_offers:
            ...
            ratio_company = SequenceMatcher(
                None, norm_inc_company, offer["company"]
            ).ratio()
            company_match = (
                ratio_company >= 0.80
                or norm_inc_company in offer["company"]
                or offer["company"] in norm_inc_company
            )
