...
async def get_usage_today() -> dict[str, Any]:
    ...
    with get_session() as s:
        ...
        total_requests = s.scalar(
            select(func.count()).select_from(GeminiUsageLog)
            .where(GeminiUsageLog.created_at >= today_start)
        ) or 0

        tokens_in = s.scalar(
            select(func.coalesce(func.sum(GeminiUsageLog.tokens_in), 0))
            .where(GeminiUsageLog.created_at >= today_start)
        ) or 0

        tokens_out = s.scalar(
            select(func.coalesce(func.sum(GeminiUsageLog.tokens_out), 0))
            .where(GeminiUsageLog.created_at >= today_start)
        ) or 0
        ...
        cache_entries = s.scalar(
            select(func.count()).select_from(GeminiCache)
        ) or 0
        ...
        by_op = s.execute(
            select(
                ...
                func.coalesce(func.sum(GeminiUsageLog.tokens_in + GeminiUsageLog.tokens_out), 0).label("tokens"),
            )
            .where(GeminiUsageLog.created_at >= today_start)
    ...
    return {
        ...
        "limits": {
            ...
            "token_pct": round(total_tokens / token_limit * 100, 2) if token_limit else 0,
            "request_pct": round(total_requests / request_limit * 100, 2) if request_limit else 0,
        },
        "by_operation": [
